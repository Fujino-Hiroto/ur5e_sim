// coarse_to_fine_go_to_leaf4.cpp
//
// Coarse->Fine approach to a leaf target using MoveIt2, TF and a PointCloud2 “forward ROI” monitor.
// Patch: Segmented motion tries Cartesian first; fallback to OMPL per segment.
//
// Phases:
// - COARSE: move to a pre-approach position (down + back) while roughly keeping a sensible orientation.
// - FINE:   move to leaf_target_shot (prefer Cartesian if available).
// - VIEW:   rotate in place to match leaf_target_view orientation.
//
// Segmented motion (enable_segmented_motion:=true):
// - We move in small position increments (segment_max_m) toward the final position.
// - To avoid “each segment spins around to a different IK branch”, we DO NOT recompute a fresh look-at
//   orientation per segment. Instead we compute a single final orientation and interpolate (SLERP)
//   from start -> goal as we progress.
//
// IMPORTANT (Node split to avoid executor conflict):
// - app_node: TF / PointCloud monitor / params / control logic
// - moveit_node: MoveGroupInterface only
//
//
// NOTE: Do NOT call rclcpp::spin_some(...) anywhere.
...
static std::mutex g_move_group_mtx;

// ============================================================
// IMPORTANT: Thread-safety policy for MoveGroupInterface
//
// - DO NOT hold g_move_group_mtx across long-running execute(...).
//   If execute() is protected by the same mutex, the monitor thread cannot call stop()
//   while execution is running (stop will block), defeating runtime monitoring.
//
// - We use g_move_group_mtx only to serialize "state-mutating" calls that configure planning:
//   setStartStateToCurrentState(), setPoseTarget()/setPositionTarget(), clearPoseTargets(),
//   setPathConstraints()/clearPathConstraints(), computeCartesianPath(), plan(), etc.
//
// - During execution, only stop() may be called concurrently, and it must not be mutex-blocked.
// ============================================================
...
struct MotionCtx
{
  std::string phase; // "COARSE" / "FINE" / "VIEW"
  int attempt{-1};
  int segment{-1};
  int replan{-1};
  std::string tag;
    ...
    double wait_service_sec)
{
  ...
  // Do NOT spin_until_future_complete; we already have an executor thread
  if (fut.wait_for(sec_to_ns(wait_service_sec)) != std::future_status::ready)
  {
    RCLCPP_WARN(logger, "RTAB-Map set_parameters timed out (%.2fs).", wait_service_sec);
    return false;
  }
...
private:
  void on_cloud(const sensor_msgs::msg::PointCloud2::SharedPtr &msg)
  {
    ...
    for (size_t i = 0; i < npts; ++i)
    {
      ...
      if (z <= 0.0f)
        continue;
      if (z > static_cast<float>(max_z_m_))
        continue;

      if (std::fabs(x) > static_cast<float>(roi_x_m_))
        continue;
      if (std::fabs(y) > static_cast<float>(roi_y_m_))
        continue;
    ...
    double tol_rad)
{
  ...
  jc.position = norm_angle(center_rad);
  jc.tolerance_above = std::fabs(tol_rad);
...
// ============================================================
// Stop policy helpers (monitor gating + leaf ignore)
// ============================================================
    ...
    bool &ignore_as_leaf_out)
{
  ...
  {
    if (dist_to_leaf < dist_near_m)
      stop_dist_eff = stop_dist_near_m;
    else if (dist_to_leaf < dist_mid_m)
      stop_dist_eff = stop_dist_mid_m;
  }
...
// ============================================================
// Plan + Execute with optional runtime monitor (stop/replan)
// ============================================================
    ...
    bool position_only = true)
{
  ...
  while (rclcpp::ok())
  {
    if (verbose_motion_log)
    {
      ...
      try
      {
        Tw_leaf = tf_buffer.lookupTransform(world_frame, leaf_center_frame, tf2::TimePointZero);
        have_leaf = true;
      }
      catch (...)
      {
      ...
      try
      {
        Tw_cam = tf_buffer.lookupTransform(world_frame, ee_link_for_target, tf2::TimePointZero);
        have_cam = true;
      }
      catch (...)
      {
      ...
      if (have_leaf)
        log_tf_xyz_q(logger, "  Tw_leaf", Tw_leaf);
      if (have_cam)
        log_tf_xyz_q(logger, "  Tw_cam", Tw_cam);
    ...
    {
      std::lock_guard<std::mutex> lk(g_move_group_mtx);
      move_group.clearPoseTargets();
      move_group.setStartStateToCurrentState();
        if (position_only) {
          move_group.setPositionTarget(goal.pose.position.x,
    ...
    std::thread exec_thread([&]()
      ...
      // NOTE: do NOT mutex-guard execute(); monitor thread must be able to call stop() concurrently.
    ...
    while (rclcpp::ok() && !exec_done.load())
    {
      ...
      if (enable_runtime_monitor && obstacle_monitor)
      {
        ...
        if (!stale && std::isfinite(min_forward) && !ignore_as_leaf && (min_forward < stop_dist_eff))
        {
          ...
          // NOTE: do NOT mutex-guard stop(); it must interrupt execute() promptly.
          move_group.stop();
          break;
        }
      }
...
// ============================================================
// Segmented motion: try Cartesian first, fallback to OMPL
// - Cartesian waypoint: move to target position (optionally hold current EE orientation)
// - If fraction >= min_fraction and execution succeeds -> segment success
// - Else -> fallback caller can run OMPL plan/execute for that segment
// ============================================================
static bool try_cartesian_segment_then_execute(
    const rclcpp::Node::SharedPtr &app_node,
    const rclcpp::Logger &logger,
    tf2_ros::Buffer &tf_buffer,
    const std::string &world_frame,
    ...
    const std::string &ee_link_for_target,
    moveit::planning_interface::MoveGroupInterface &move_group,
    const geometry_msgs::msg::PoseStamped &seg_goal_ee,   // pose in world_frame for ee_link
    ForwardObstacleMonitor *obstacle_monitor,
    bool enable_runtime_monitor,
    double monitor_rate_hz,
    double stop_dist_m,
    ...
    double leaf_ignore_band_m,
    // Cartesian knobs
    double cart_eef_step,
    double cart_jump_thr,
    double cart_min_fraction,
    bool cart_hold_current_orientation,
    const geometry_msgs::msg::Quaternion &cart_hold_q,
    // Logging context
    const MotionCtx &ctx,
    double &out_fraction)
{
  ...
  // Build waypoint
  geometry_msgs::msg::Pose wp = seg_goal_ee.pose;
  if (cart_hold_current_orientation)
  {
    // Hold caller-provided orientation (do not call getCurrentPose() here).
    // This avoids per-call behavior changes and reduces MoveGroupInterface API usage.
    wp.orientation = cart_hold_q;
  }
...

  {
    std::lock_guard<std::mutex> lk(g_move_group_mtx);
    move_group.setStartStateToCurrentState();
    out_fraction = move_group.computeCartesianPath(
        waypoints, cart_eef_step, cart_jump_thr, traj, /*avoid_collisions*/ true);
  }

  if (out_fraction < cart_min_fraction)
  {
    RCLCPP_INFO(logger,
                "[%s][a=%d][seg=%d][replan=%d][%s] SEG-CART: fraction=%.3f < min=%.3f -> fallback to OMPL",
                ctx.phase.c_str(), ctx.attempt, ctx.segment, ctx.replan, ctx.tag.c_str(),
                out_fraction, cart_min_fraction);
    return false;
  }

  RCLCPP_INFO(logger,
              "[%s][a=%d][seg=%d][replan=%d][%s] SEG-CART: fraction=%.3f >= min=%.3f -> exec",
              ctx.phase.c_str(), ctx.attempt, ctx.segment, ctx.replan, ctx.tag.c_str(),
              out_fraction, cart_min_fraction);

  const bool exec_ok = execute_traj_with_monitor_gated(
      app_node, logger,
      tf_buffer, world_frame, leaf_center_frame, ee_link_for_target,
      move_group,
      traj,
      obstacle_monitor,
  ...
  {
    RCLCPP_WARN(logger,
                "[%s][a=%d][seg=%d][replan=%d][%s] SEG-CART: exec failed/aborted -> fallback to OMPL",
                ctx.phase.c_str(), ctx.attempt, ctx.segment, ctx.replan, ctx.tag.c_str());
    return false;
  }

  RCLCPP_INFO(logger,
              "[%s][a=%d][seg=%d][replan=%d][%s] SEG-CART: exec OK",
              ctx.phase.c_str(), ctx.attempt, ctx.segment, ctx.replan, ctx.tag.c_str());
  return true;
}

// ============================================================
// Segmented motion helper (position increments + orientation SLERP)
// ============================================================
static bool move_segmented_lookat(
    ...
    bool log_wrist1_angle,
    const std::string &wrist1_joint_name,
    // Orientation constraint knobs (kept, but we default-disable in COARSE via a param in main)
    bool enable_orientation_path_constraint,
    ...
    // Seg-Cartesian first
    bool enable_segment_cartesian,
    bool segment_cart_hold_current_orientation,
    double seg_cart_eef_step,
    double seg_cart_jump_thr,
    double seg_cart_min_fraction,
    // New: SLERP control
    bool enable_segment_orientation_slerp,
    double slerp_alpha_power,
    bool freeze_leaf_during_segments)
{
  // --- capture start pose/orientation once ---
// 1) leaf TF (必要ならfreezeするが、初回取得は必須)
  ...
  if (!lookup_tf_blocking(tf_buffer, app_node, world_frame, leaf_center_frame, Tw_leaf, tf_timeout_sec)) {
      RCLCPP_ERROR(logger, "TF timeout: %s <- %s (waited %.2fs)",
               world_frame.c_str(), leaf_center_frame.c_str(), tf_timeout_sec);
      return false;
  }
  geometry_msgs::msg::Vector3 leaf_pos_world = pos_from_tf(Tw_leaf);

  // 2) start pose（camera_frameがあればそれ基準で）
  geometry_msgs::msg::PoseStamped start_ps;
  {
    const std::string start_link = camera_frame.empty() ? ee_link : camera_frame;
    start_ps = move_group.getCurrentPose(start_link);
  }
  const geometry_msgs::msg::Quaternion q_start_msg = start_ps.pose.orientation;

  // 3) goal orientation（final posで1回だけ look-at）
  const geometry_msgs::msg::Vector3 world_up = v3(0,0,1);
  const auto final_look = make_lookat_pose(world_frame, leaf_pos_world, world_up, final_pos_world);
  const geometry_msgs::msg::Quaternion q_goal_msg = final_look.pose.orientation;

  // 4) rem0（start->final の距離）
  const geometry_msgs::msg::Vector3 start_pos_world =
    v3(start_ps.pose.position.x, start_ps.pose.position.y, start_ps.pose.position.z);
  const double rem0 = std::max(1e-6, v3_norm(v3_sub(final_pos_world, start_pos_world)));

  // 5) tf2 quaternion準備
  tf2::Quaternion q_start, q_goal;
  tf2::fromMsg(q_start_msg, q_start);
  tf2::fromMsg(q_goal_msg, q_goal);
  q_start.normalize(); q_goal.normalize();


  RCLCPP_INFO(logger,
              "[SEG] start: final_pos=(%.3f,%.3f,%.3f) leaf=(%.3f,%.3f,%.3f) seg_max=%.3f tol=%.3f rem0=%.3f slerp=%s seg_cart=%s",
              final_pos_world.x, final_pos_world.y, final_pos_world.z,
              leaf_pos_world.x, leaf_pos_world.y, leaf_pos_world.z,
              segment_max_m, arrive_pos_tol_m, rem0,
              tf_bool(enable_segment_orientation_slerp),
              tf_bool(enable_segment_cartesian));

  bool ever_set_orient_constraint = false;

  for (int iter = 0; rclcpp::ok(); ++iter)
  {
    const std::string cur_link = camera_frame.empty() ? ee_link : camera_frame;
    const auto cur_ps = move_group.getCurrentPose(cur_link);
    const geometry_msgs::msg::Vector3 cur_pos =
    ...
    seg_goal.header.frame_id = world_frame;
    seg_goal.pose.position.x = seg_pos.x;
    seg_goal.pose.position.y = seg_pos.y;
    seg_goal.pose.position.z = seg_pos.z;

    if (enable_segment_orientation_slerp)
    {
      // progress alpha: 0 at start -> 1 near goal
      double alpha = 1.0 - (rem / rem0);
      alpha = clamp01(alpha);

      // optional shaping (alpha^p), p>1 slows early rotation; p<1 rotates earlier
      if (slerp_alpha_power > 0.0 && std::fabs(slerp_alpha_power - 1.0) > 1e-6)
        alpha = std::pow(alpha, slerp_alpha_power);

      tf2::Quaternion q_interp = q_start.slerp(q_goal, alpha);
      q_interp.normalize();
      seg_goal.pose.orientation = tf2::toMsg(q_interp);
    }
    else
    {
      // fallback: classic per-segment look-at (can cause large IK branch jumps)
      seg_goal = make_lookat_pose(world_frame, leaf_pos_world, world_up, seg_pos);
    }
    ...
    // --- Optional path constraint (generally NOT recommended for COARSE stability) ---
    bool apply_orient_constraint = false;
    if (enable_orientation_path_constraint)
    {
    ...
    if (apply_orient_constraint)
    {
      ...
      if (verbose_motion_log)
      {
        RCLCPP_INFO(logger,
                    ...
                    ee_orient_tol_x_rad * 180.0 / M_PI,
                    ee_orient_tol_y_rad * 180.0 / M_PI,
                    ee_orient_tol_z_rad * 180.0 / M_PI,
                    rem);
      }
    }
    else
    ...
    geometry_msgs::msg::PoseStamped seg_goal_ee;
    (void)convert_goal_pose_for_ee(tf_buffer, logger, camera_frame, ee_link, seg_goal, seg_goal_ee);

    // ============================================================
    // 1) Try Cartesian first (per-segment), fallback to OMPL on failure
    // ============================================================
  bool seg_done = false;

  if (enable_segment_cartesian)
  {
    // Decide "hold orientation" once per segment (best-effort).
    // Prefer TF-based orientation to avoid MoveGroupInterface calls and to keep behavior consistent.
    geometry_msgs::msg::Quaternion cart_hold_q = seg_goal_ee.pose.orientation;
    if (segment_cart_hold_current_orientation)
    {
      try
      {
        const auto Tw_ee = tf_buffer.lookupTransform(world_frame, ee_link, tf2::TimePointZero);
        cart_hold_q = Tw_ee.transform.rotation;
      }
      catch (...)
      {
        // Keep seg_goal_ee orientation as fallback.
      }
    }

    double frac = 0.0;
    const bool cart_ok = try_cartesian_segment_then_execute(
        app_node, logger, tf_buffer,
        world_frame, leaf_center_frame,
        ee_link,
        move_group,
        seg_goal_ee,
        obstacle_monitor,
        ...
        seg_cart_eef_step,
        seg_cart_jump_thr,
        seg_cart_min_fraction,
        segment_cart_hold_current_orientation,
        cart_hold_q,
        ctx,
        frac);
  ...
  if (!seg_done)
  {
    ...
    // leaf TF refresh (optional): for COARSE stability, freezing often reduces jitter/IK flips
    if (!freeze_leaf_during_segments)
    {
      if (!lookup_tf_blocking(tf_buffer, app_node, world_frame, leaf_center_frame, Tw_leaf, tf_timeout_sec))
      {
...
int main(int argc, char **argv)
{
  ...
  // Hold orientation for FINE Cartesian (set after PRE-ORIENT if succeeded)
  geometry_msgs::msg::Quaternion q_fine_hold;
  bool have_q_fine_hold = false;

  // ============================================================
  // Node split
  // ============================================================
  auto app_node = rclcpp::Node::make_shared("coarse_to_fine_go_to_leaf4");
  const auto logger = app_node->get_logger();

  // MoveIt node (do NOT add to our executor)
  auto moveit_node = rclcpp::Node::make_shared(
      "coarse_to_fine_go_to_leaf4_moveit",
      rclcpp::NodeOptions().allow_undeclared_parameters(true).automatically_declare_parameters_from_overrides(true));

  // ============================================================
  // Executor (app_node only)
  // ============================================================
  ...
  // ============================================================
  // use_sim_time (declare on app_node; propagate to moveit_node)
  // ============================================================
  ...
  // ============================================================
  // Copy MoveIt description params from move_group into moveit_node
  // ============================================================
  {
    auto params_client = std::make_shared<rclcpp::AsyncParametersClient>(app_node, "move_group");
    if (params_client->wait_for_service(std::chrono::seconds(5)))
    {
      std::set<std::string> names;
      auto add_prefix = [&](const std::string &prefix)
      {
        auto list_fut = params_client->list_parameters({prefix}, 10);
        if (list_fut.wait_for(sec_to_ns(2.0)) != std::future_status::ready)
        {
      ...
      if (!names.empty())
      {
        ...
        if (get_fut.wait_for(sec_to_ns(2.0)) == std::future_status::ready)
        {
      ...
      {
        RCLCPP_WARN(logger, "No MoveIt description parameters found on /move_group.");
      }
    }
    else
    {
  ...
  // IMPORTANT: ee_link must exist in BOTH TF and robot model (for MoveIt)
  const std::string ee_link = app_node->declare_parameter<std::string>("ee_link", "tool0");

  const double switch_goal_m = app_node->declare_parameter<double>("switch_goal_m", 0.60);
  const double switch_plant_m = app_node->declare_parameter<double>("switch_plant_m", 0.50);
  ...
  const bool exit_after_coarse =
    app_node->declare_parameter<bool>("exit_after_coarse", false);
  ...
  const double pre_back_m = app_node->declare_parameter<double>("pre_back_m", 0.20);
  const double pre_down_m = app_node->declare_parameter<double>("pre_down_m", 0.15);

  const bool enable_segmented_motion =
      app_node->declare_parameter<bool>("enable_segmented_motion", true);
  const double segment_max_m =
      app_node->declare_parameter<double>("segment_max_m", 0.15);
  const double arrive_pos_tol_m =
      app_node->declare_parameter<double>("arrive_pos_tol_m", 0.02);

  // New: segmented orientation SLERP controls (recommended for COARSE stability)
  const bool enable_segment_orientation_slerp =
      app_node->declare_parameter<bool>("enable_segment_orientation_slerp", true);
  const double slerp_alpha_power =
      app_node->declare_parameter<double>("slerp_alpha_power", 1.8); // >1 rotates later (more stable early)
  const bool freeze_leaf_during_segments =
      app_node->declare_parameter<bool>("freeze_leaf_during_segments", true); // COARSE stability

  const bool enable_fine_cartesian =
      app_node->declare_parameter<bool>("enable_fine_cartesian", true);
  const double cart_eef_step =
      app_node->declare_parameter<double>("cart_eef_step", 0.01);
  const double cart_jump_thr =
      app_node->declare_parameter<double>("cart_jump_threshold", 0.5);
  const double cart_min_fraction =
      app_node->declare_parameter<double>("cart_min_fraction", 0.80);

  // ============================================================
  // NEW: Segmented-Cartesian-first knobs
  // - Keep ON by default; you can disable for baseline comparisons.
  // - Hold current EE orientation by default to avoid large joint spins.
  // ============================================================
  const bool enable_segment_cartesian =
      app_node->declare_parameter<bool>("enable_segment_cartesian", true);

  const bool segment_cart_hold_current_orientation =
      app_node->declare_parameter<bool>("segment_cart_hold_current_orientation", true);

  const double seg_cart_eef_step =
      app_node->declare_parameter<double>("segment_cart_eef_step", cart_eef_step);

  const double seg_cart_jump_thr =
      app_node->declare_parameter<double>("segment_cart_jump_threshold", cart_jump_thr);

  const double seg_cart_min_fraction =
      app_node->declare_parameter<double>("segment_cart_min_fraction", cart_min_fraction);
  ...
  const bool switch_rtabmap =
      app_node->declare_parameter<bool>("switch_rtabmap", true);
  const std::string rtabmap_node_name =
      app_node->declare_parameter<std::string>("rtabmap_node_name", "/rtabmap");
  const double wait_param_service_sec =
      app_node->declare_parameter<double>("wait_param_service_sec", 5.0);
  const double fine_settle_wait_sec =
      app_node->declare_parameter<double>("fine_settle_wait_sec", 2.0);

  const double coarse_detection_rate =
      app_node->declare_parameter<double>("coarse_detection_rate", 2.0);
  const double fine_detection_rate =
      app_node->declare_parameter<double>("fine_detection_rate", 10.0);

  const bool set_grid_params =
      app_node->declare_parameter<bool>("set_grid_params", false);
  const double coarse_grid_cell =
      app_node->declare_parameter<double>("coarse_grid_cell", 0.05);
  const double fine_grid_cell =
      app_node->declare_parameter<double>("fine_grid_cell", 0.02);
  const double coarse_range_max =
      app_node->declare_parameter<double>("coarse_range_max", 3.0);
  const double fine_range_max =
      app_node->declare_parameter<double>("fine_range_max", 1.5);
  ...
  // Orientation path constraint (kept, but recommended OFF in COARSE)
  ...
  const bool disable_orientation_path_constraint_in_coarse =
      app_node->declare_parameter<bool>("disable_orientation_path_constraint_in_coarse", true);

  const int coarse_orient_from_seg =
      app_node->declare_parameter<int>("coarse_orient_from_seg", 999); // default safe
  ...
  // ============================================================
  std::shared_ptr<rclcpp::AsyncParametersClient> rtabmap_params_client;
  if (switch_rtabmap)
  {
    rtabmap_params_client = std::make_shared<rclcpp::AsyncParametersClient>(app_node, rtabmap_node_name);
  ...
  // ============================================================
  // Distance computation via TF (app_node)
  // ============================================================
  auto compute_distances = [&](double &dist_goal, double &dist_plant) -> bool
  {
    geometry_msgs::msg::TransformStamped Twc, Twg, Twp;

    const std::string dist_link = camera_frame.empty() ? ee_link : camera_frame;
    if (!lookup_tf_blocking(tf_buffer, app_node, world_frame, dist_link, Twc, tf_timeout_sec))
      return false;
    ...
    dist_plant = has_plant ? dist_xyz(Twc.transform.translation, Twp.transform.translation)
  ...
  // COARSE: pre approach
  // ============================================================
  RCLCPP_INFO(logger, "=== COARSE: pre approach (target %s) ===", leaf_center_frame.c_str());
  ...
  for (int attempt = 1; attempt <= max_coarse_attempts && rclcpp::ok(); ++attempt)
  {
    ...
    {
      // (a) Fix: avoid constant string comparison. We are in COARSE block here.
      // If you want to disable constraints during COARSE, this expresses it directly.
      const bool enable_path_constraint_for_this_phase =
          enable_late_ee_orientation_constraint && !disable_orientation_path_constraint_in_coarse;

      ok = move_segmented_lookat(
          app_node, logger, tf_buffer,
          move_group,
          world_frame, ee_link, camera_frame,
          leaf_center_frame,
          pre_pos_world,
          segment_max_m,
          arrive_pos_tol_m,
          ...
          log_wrist1_angle,
          wrist1_joint_name,
          enable_path_constraint_for_this_phase,
          ...
          ee_orient_weight,
          // seg-cart first
          enable_segment_cartesian,
          segment_cart_hold_current_orientation,
          seg_cart_eef_step,
          seg_cart_jump_thr,
          seg_cart_min_fraction,
          enable_segment_orientation_slerp,
          slerp_alpha_power,
          freeze_leaf_during_segments);
    }
    else
    {
      // single shot (not segmented)
      const geometry_msgs::msg::Vector3 world_up = v3(0, 0, 1);
      const auto goal_pose = make_lookat_pose(world_frame, leaf_pos_world, world_up, pre_pos_world);
      geometry_msgs::msg::PoseStamped goal_pose_ee;
      (void)convert_goal_pose_for_ee(tf_buffer, logger, camera_frame, ee_link, goal_pose, goal_pose_ee);
    ...
    if (cond_goal || cond_plant)
    {
      ...
      if (exit_after_coarse)
          {
            RCLCPP_INFO(logger, "exit_after_coarse=true -> Stop after COARSE. (Skipping FINE/VIEW)");
  ...
  // ============================================================
  // PRE-ORIENT: rotate in place to face leaf (POI) before FINE Cartesian
  // - This improves Cartesian fraction when camera is not facing the target.
  // ============================================================
  {
    RCLCPP_INFO(logger, "=== PRE-ORIENT: rotate in place to face leaf_target (POI) ===");

    // Rotate only => use VIEW-like slow dynamics
    ...
    geometry_msgs::msg::TransformStamped Tw_leaf2;
    if (!lookup_tf_blocking(tf_buffer, app_node, world_frame, leaf_center_frame, Tw_leaf2, tf_timeout_sec))
    {
      RCLCPP_WARN(logger, "[PRE-ORIENT] TF timeout: %s <- %s. Skip PRE-ORIENT.",
                  world_frame.c_str(), leaf_center_frame.c_str());
    }
    else
    {
      const auto leaf_pos_world2 = pos_from_tf(Tw_leaf2);

      // Current camera pose in world
      const auto cur_cam_ps = move_group.getCurrentPose(camera_frame);
      geometry_msgs::msg::Vector3 cur_cam_pos =
          v3(cur_cam_ps.pose.position.x, cur_cam_ps.pose.position.y, cur_cam_ps.pose.position.z);

      // Build "look-at" orientation at current position
      const geometry_msgs::msg::Vector3 world_up = v3(0, 0, 1);
      auto look_pose = make_lookat_pose(world_frame, leaf_pos_world2, world_up, cur_cam_pos);

      geometry_msgs::msg::PoseStamped goal_cam;
      goal_cam.header.frame_id = world_frame;
      goal_cam.pose.position = cur_cam_ps.pose.position;      // position fixed
      goal_cam.pose.orientation = look_pose.pose.orientation; // orientation updated

      // Convert camera goal -> ee goal
      geometry_msgs::msg::PoseStamped goal_ee;
      (void)convert_goal_pose_for_ee(tf_buffer, logger, camera_frame, ee_link, goal_cam, goal_ee);

      MotionCtx ctx;
      ctx.phase = "PRE_ORIENT";
      ctx.attempt = -1;
      ctx.segment = -1;
      ctx.replan = 0;
      ctx.tag = "poi";
      ...
      const bool ok = plan_execute_with_runtime_replan(
          ...
          goal_ee, ee_link,
          &obstacle_monitor,
          /*enable_runtime_monitor=*/ false, // disable ROI stop during rotation (recommended)
          monitor_rate_hz,
          stop_dist_m,
          pc_stale_sec,
          max_runtime_replans,
      ...
      if (!ok)
      {
        RCLCPP_WARN(logger, "[PRE-ORIENT] rotate-in-place failed. Continue anyway.");
      }
      else
      {
        RCLCPP_INFO(logger, "[PRE-ORIENT] rotate-in-place success.");
        // Save the post-rotation EE orientation as the "hold" orientation for FINE Cartesian
        q_fine_hold = move_group.getCurrentPose(ee_link).pose.orientation;
        have_q_fine_hold = true;
      }
    }
  }
  ...
  // ============================================================
  // FINE: approach shot pose (leaf_target_shot)
  // ============================================================
  RCLCPP_INFO(logger, "=== FINE: go to %s ===", fine_goal_frame.c_str());
  ...
  if (enable_fine_cartesian)
  {
    for (int k = 0; rclcpp::ok(); ++k)
    {
      ...
      geometry_msgs::msg::PoseStamped shot_pose_ee;
      (void)convert_goal_pose_for_ee(tf_buffer, logger, camera_frame, ee_link, shot_pose, shot_pose_ee);

      // Hold orientation decided at PRE-ORIENT (preferred). Fallback: current EE orientation.
      if (have_q_fine_hold)
      {
        shot_pose_ee.pose.orientation = q_fine_hold;
      }
      else
      {
        shot_pose_ee.pose.orientation = move_group.getCurrentPose(ee_link).pose.orientation;
      }

      // Build waypoints (single waypoint to the EE pose in world_frame)
      std::vector<geometry_msgs::msg::Pose> waypoints;
      waypoints.reserve(1);
      waypoints.push_back(shot_pose_ee.pose);
  ...
  if (!fine_ok)
  {
    if (enable_segmented_motion)
    {
      fine_ok = move_segmented_lookat(
          ...
          log_wrist1_angle,
          wrist1_joint_name,
          enable_late_ee_orientation_constraint, // allow in FINE if desired
          coarse_orient_from_seg,
          fine_orient_from_seg,
          orient_enable_rem_m,
          ee_orient_tol_x_rad,
          ...
          enable_segment_orientation_slerp,
          slerp_alpha_power,
          /*freeze_leaf_during_segments*/ false);
    }
    else
    {
      const auto shot_pose = make_lookat_pose(world_frame, leaf_pos_world, world_up, shot_pos_world);
  ...
  // ============================================================
  // VIEW: orientation-only alignment at current position (best-effort)
  // ============================================================
  {
    RCLCPP_INFO(logger, "=== VIEW: orientation-only align to %s (position fixed) ===", view_goal_frame.c_str());

    // 速度は低め（あなたの view_vel_scale / view_acc_scale を使用）
    move_group.setPlanningTime(/*例*/ 3.0);
    move_group.setMaxVelocityScalingFactor(view_vel);
    move_group.setMaxAccelerationScalingFactor(view_acc);

    // 1) 現在の camera_frame の pose（world基準）を取得
    const auto cur_cam_ps = move_group.getCurrentPose(camera_frame);

    // 2) leaf_target_view の姿勢（world基準）を TF から取得
    ...
    {
      RCLCPP_WARN(logger, "[VIEW] TF timeout: %s <- %s. Skip VIEW.", world_frame.c_str(), view_goal_frame.c_str());
    }
    else
    {
      // 3) “位置固定＋姿勢だけ置換”した目標（camera_frame の目標姿勢）を作る
      geometry_msgs::msg::PoseStamped view_goal_cam;
      view_goal_cam.header.frame_id = world_frame;
      view_goal_cam.pose.position = cur_cam_ps.pose.position;              // 位置固定
      view_goal_cam.pose.orientation = Tw_view.transform.rotation;          // 姿勢だけ合わせる

      // 4) camera_frame目標 -> ee_link目標へ変換（tool0ターゲットに落とす）
      geometry_msgs::msg::PoseStamped view_goal_ee;
      (void)convert_goal_pose_for_ee(tf_buffer, logger, camera_frame, ee_link, view_goal_cam, view_goal_ee);

      // 5) Plan+Execute（ここは orientation を効かせたいので position_only=false）
      MotionCtx ctx;
      ctx.phase = "VIEW";
      ctx.attempt = -1;
      ctx.segment = -1;
      ...
      const bool view_ok = plan_execute_with_runtime_replan(
            ...
            tf_buffer, world_frame, leaf_center_frame,
            move_group,
            view_goal_ee, ee_link,
            &obstacle_monitor,
            /*enable_runtime_monitor=*/ false,
            monitor_rate_hz,
            stop_dist_m,
            pc_stale_sec,
            max_runtime_replans,
      ...
      if (!view_ok)
      {
        RCLCPP_WARN(logger, "[VIEW] Orientation-only align failed. Continue anyway (best-effort).");
      }
      else
      {
        RCLCPP_INFO(logger, "[VIEW] Orientation-only align success.");
      }
    }
  }
  ...
  RCLCPP_INFO(logger, "DONE: coarse(pre) -> fine(shot) finished.");
  (void)camera_frame; // kept for parameter compatibility
  return cleanup_and_exit(0);
}
